...
row = 1
for item, price in menu_items.items():
    tk.Label(frame_menu, text=f"{item} (₹{price})", font=("Arial", 14), bg="beige").grid(row=row, column=0, sticky="w", pady=5)
...
# Buttons
btn_total = tk.Button(frame_buttons, text="Calculate Total", font=("Arial", 14, "bold"), bg="green", fg="white", width=15, command=calculate_total)
btn_total.grid(row=0, column=0, padx=10)

btn_receipt = tk.Button(frame_buttons, text="Generate Receipt", font=("Arial", 14, "bold"), bg="blue", fg="white", width=18,
                        command=lambda: [generate_receipt(), receipt_box.delete(1.0, "end"), receipt_box.insert("end", receipt_text.get())])
btn_receipt.grid(row=0, column=1, padx=10)

btn_reset = tk.Button(frame_buttons, text="Reset", font=("Arial", 14, "bold"), bg="orange", fg="white", width=10, command=reset_order)
btn_reset.grid(row=0, column=2, padx=10)

btn_exit = tk.Button(frame_buttons, text="Exit", font=("Arial", 14, "bold"), bg="red", fg="white", width=10, command=exit_app)
